...
export class UsuariosService {
  ...
  private completarConRoles(usuarioSesion: UsuarioSesion): Observable<UsuarioSesion> {
    ...
    if(usuarioSesion.id == 6){
      usuarioSesion.roles.push({rol: Rol.ENTRENADOR, centro: 1, nombreCentro: 'Málaga'});
    } else {
      usuarioSesion.roles.push({rol: Rol.CLIENTE, centro: 1, nombreCentro: 'Málaga'});
    }
